/**
 * Copied from https://github.com/1uphealth/1upwebapp/blob/master/oneup.js
 */
...
function getTokenFromAuthCode(code, callback) {
  var postUrl = `${ROOT_API_URL}/fhir/oauth2/token?client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&code=${code}&grant_type=authorization_code`;
...
function getAuthCodeForExistingUser(email, callback) {
  request.post(
    {
      url: `${ROOT_API_URL}/user-management/v1/user/auth-code?app_user_id=${email}&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}`,
    },
    (error, response, body) => {
...
function createOneUpUser(email, callback) {
  let url = `${USER_API_URL}/user-management/v1/user?app_user_id=${email}&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}`;
...
function getOneUpUserId(email, callback) {
  let getUrl = `${USER_API_URL}/user-management/v1/user?client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}&app_user_id=${email}`;
  console.log(getUrl);
